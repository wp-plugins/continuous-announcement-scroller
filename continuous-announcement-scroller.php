...
function cas_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_cas_TABLE . "'") != WP_cas_TABLE) 
	{
		...
		$wpdb->query($sSql);
		$sSql = "INSERT INTO `". WP_cas_TABLE . "` (`cas_text`,`cas_link`, `cas_order`, `cas_status`, `cas_date`)"; 
		$sSql = $sSql . "VALUES ('To updae the setting use plugin link under wordpress setting tab.','http://www.gopiplus.com/work/', '2', 'YES', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		$sSql = "INSERT INTO `". WP_cas_TABLE . "` (`cas_text`,`cas_link`, `cas_order`, `cas_status`, `cas_date`)"; 
		$sSql = $sSql . "VALUES ('Click here to see more help, comments for this wordspress plugin.','http://www.gopiplus.com/work/', '3', 'YES', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
	}
...
function cas_add_to_menu() 
{
	...
	add_options_page('Continuous announcement scroller', '', 0, "continuous-announcement-scroller/content-management.php",'' );
}
